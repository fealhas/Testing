...
elFinder.prototype.commands.quicklook = function() {
	var self       = this,
			...
			.mousedown(function(e) {
				...
				if (full) {
					...
					.bind(self.resize, function(e) {	
							a = preview														
                        $('#zoom').children('.loader').remove();
						$('#zoom').children('img').attr('style','');
						var img = $('#zoom');
						...
						var preview = $('.elfinder-quicklook-preview')
						preview.html(imgHtml);
						
						img = preview.children('img');
						
						img.height(preview.height()).width('auto');
						
						a = preview														
							.html('<div id="zoom" style="position:relative !important">'+preview.html()+'</div>');
							$('#zoom').smoothZoom({
								width: img.width(),
								height: img.height()-48,
							});	

							//.children('img').css('opacity', '100');
							//.parents('.elfinder-quicklook').css('width','auto')
							//.parents('.elfinder-quicklook').css('height','auto');
		...
		navbar  = $('<div class="elfinder-quicklook-navbar"/>')
			...
			.append($('<div class="'+navicon+' '+navicon+'-close"/>').mousedown(function() { self.window.trigger('close'); self.trigger('changesize'); }))
		;
	...
	this.init = function() {
		...
		height = o.height > 0 ? parseInt(o.height) : 600;
