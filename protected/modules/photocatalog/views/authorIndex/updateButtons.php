...
$isEditor = Yii::app()->user->checkAccess('Photocatalog.Data.Update');
// если юзер из центра, контроль отображения editor_buttons будет при переходе по дереву
...
echo "<script>
		      $(document).ready(function() {
  		          if ('$isCentr' || '$isFilal') $('.editor_button').hide(); 
		      });
      </script>";
if ($isCentr || $isFilal) $isEditor=true;
//
...
<div class='toolstrip'>
    ...
    <div class='row buttons' id='createToolbar' style='display: none;'>
        ...
        echo CHtml::imageButton("$base_path/img/icons/actions/attachment.png", array(
                        "onClick"=>"createRecordHide(); return false;",
                        'class'=>'action_button',
			'id'=>'attachCreateButton',
    ...
    <div class="row buttons" id='updateToolbar'>
        ...
        ?>
        <div class="editor_button" style="display:inline-block; height: 32px; border-left: 1px solid black;"></div>
        <?php
        }
        ?>
        <?php
...
//            $disableDel = false;
 if ($isEditor===true){
       ...
       <div style="display:inline-block; height: 32px; border-left: 1px solid black;"></div>
        ...
        echo CHtml::imageButton("$base_path/img/icons/actions/word.png", array(
            "onClick" => "getCard(); return false;",
            'class' => 'action_button',
            'id' => 'wordBtn',
            'title' => 'Карточка',
            /*'target' => '_blank', */
