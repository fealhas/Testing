<div class="bloc abs">
    <div class="title">
            Фотокаталог
            <div class='tabs'>
            <a href="#author_index" id='auth_index'>Главное меню</a>
	...
	<?php
	$id = Data::model()->getFirstId();
	$model = Data::model()->findByPk($id);
	
	$this->renderPartial('application.modules.photocatalog.views.authorIndex.index', array(
			     'model'=>$model,
			     ));
	?>
    </div>
	...
	<?php
	$model = new Data();
	$this->renderPartial('application.modules.photocatalog.views.authorIndex.search',
			     array(
				   'model'=>$model
				  )
			     );
	...
	$this->renderPartial('application.modules.photocatalog.views.dictionary.index',
			     array('model'=>$model));
	?>
    </div>
</div>
