<?php
class PhotocatalogModule extends CWebModule
{
	public $juiScriptUrl;
	/**
	...
	*/
	...
	public function init()
	{
		$this->setImport(array(
			'photocatalog.models.*',
			'photocatalog.components.SDynaTree.*',
			'photocatalog.controllers',
		));
	...
	public function _publishModuleAssets()
	{
		...
		if (!is_string($this->baseScriptUrl)) {
			...
			$this->baseScriptUrl = ($this->development ?
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('photocatalog.assets'), false, -1, true) :
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('photocatalog.assets'))
			);
		}
