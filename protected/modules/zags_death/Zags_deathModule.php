<?php
class Zags_deathModule extends CWebModule
{
	public $juiScriptUrl;
	/**
	...
	*/
	...
	public function init()
	{
		$this->setImport(array(
			'zags_death.models.*',
			'zags_death.components.SDynaTree.*',
			'zags_death.controllers',
		));
	...
	public function _publishModuleAssets()
	{
		...
		if (!is_string($this->baseScriptUrl)) {
			...
			$this->baseScriptUrl = ($this->development ?
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('zags_death.assets'), false, -1, true) :
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('zags_death.assets'))
			);
		}
