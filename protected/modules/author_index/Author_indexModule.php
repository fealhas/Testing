<?php
class Author_indexModule extends CWebModule
{
	public $juiScriptUrl;
	/**
	...
	*/
	...
	public function init()
	{
		$this->setImport(array(
			'author_index.models.*',
			'author_index.components.SDynaTree.*',
			'author_index.controllers',
		));
	...
	public function _publishModuleAssets()
	{
		...
		if (!is_string($this->baseScriptUrl)) {
			...
			$this->baseScriptUrl = ($this->development ?
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('author_index.assets'), false, -1, true) :
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('author_index.assets'))
			);
		}
		...
		//$this->_cs->registerScriptFile('/elfinder2/js/jquery-1.7.1.min.js');
