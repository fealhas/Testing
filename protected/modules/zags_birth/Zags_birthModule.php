<?php
class Zags_birthModule extends CWebModule
{
	public $juiScriptUrl;
	/**
	...
	*/
	...
	public function init()
	{
		$this->setImport(array(
			'zags_birth.models.*',
			'zags_birth.components.SDynaTree.*',
			'zags_birth.controllers',
		));
	...
	public function _publishModuleAssets()
	{
		...
		if (!is_string($this->baseScriptUrl)) {
			...
			$this->baseScriptUrl = ($this->development ?
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('zags_birth.assets'), false, -1, true) :
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('zags_birth.assets'))
			);
		}
