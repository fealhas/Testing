<?php
class SyscatalogModule extends CWebModule
{
	public $juiScriptUrl;
	/**
	...
	*/
	...
	public function init()
	{
		$this->setImport(array(
			'syscatalog.models.*',
			'syscatalog.components.SDynaTree.*',
			'syscatalog.controllers',
		));
	...
	public function _publishModuleAssets()
	{
		...
		if (!is_string($this->baseScriptUrl)) {
			...
			$this->baseScriptUrl = ($this->development ?
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('syscatalog.assets'), false, -1, true) :
				Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('syscatalog.assets'))
			);
		}
